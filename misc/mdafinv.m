function inv = mdafinv (x, m, n)
% MDAFINV inverse CDF for F-distribution
...
% ## Description: Quantile function of the F distribution
  ...
  if (nargin ~= 3)
     error('finv: three arguments are expected!')
  end
  ...
  if (isscalar (m) && isscalar (n))
    inv(k) = ((1 ./ betaincinv (1 - x(k), n/2, m/2) - 1) * n / m);
  else
    inv(k) = ((1 ./ betaincinv (1 - x(k), n(k)/2, m(k)/2) - 1) .* n(k) ./ m(k));
  end
